...
3. Under the major version subdirectory, the next level includes a "schema" subdirectory, an "example" subdirectory as well as the OpenAPI Specification files.  The "schema" subdirectory contains the message payload schema files.  The "example" subdirectory contains example instances of the message payload schemas.
...
An exception to the above is the the transport-objects subdirectory. This directory contains JSON objects that are subsets of the larger set. The objects are used for the special case of transporting the objects via a specific infrastructure.  
